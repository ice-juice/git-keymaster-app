...
> **本地加密的 Git 多身份工作站**：把 SSH 密钥、Host 别名、ssh-agent、提交身份、仓库归属，以及 2FA、隐私账号、文件附件与备忘录收进同一套保险库，在 Windows / macOS / Linux 上开箱使用。
...
- 密钥命名混乱，口令写在便签、聊天记录或不加密的文件里
- `~/.ssh/config` 漏配 `IdentitiesOnly yes`，认证串号，严重时被平台拉黑
- Windows 上 `ssh-agent` 停了就要重新输口令；终端、编辑器和 Git 还可能各走一套 agent
- 仓库忘了设局部 `user.name` / `user.email`，商业提交挂上个人邮箱
- 换机时用网盘或 U 盘拷私钥，既不安全，也没有成体系的加密备份

Git 配好了，旁边的机密往往仍是一盘散沙：

- **隐私账号**：同一平台多个号（GitHub 工作/个人、云控制台、邮箱）密码记混，改密后找不到上一版，登录还要另开一个验证器
- **文件保险库**：护照扫描件、恢复码截图、合同授权书、旧私钥备份要么裸放文件夹，要么锁进某个网盘，无法和工作空间一起换机
- **备忘录**：跳板机信息、机房位置、操作步骤和截图散在聊天或备忘 App 里，和密钥、账号对不上号

御钥师把这些收进同一套本机保险库：

- **一个 Git 身份**是可管理对象——密钥、Host 别名、agent、提交身份、关联仓库在一个界面，并能一键体检和修复
- **隐私账号**按平台聚合，密码二次验证、保留历史，并可联动本机 2FA
- **文件保险库**加密存放证件、备份和任意附件，自定义名称与备注，需要时再解密导出
- **备忘录**用 Markdown 记私密图文（环境说明、操作手册），图片随笔记加密，跟着同一把恢复密钥走

密钥、密码、附件和笔记只在本机加密保管；云同步只把密文放到**你自己的** S3 / R2，我们不运营中心服务器。
...
- **文件保险库**：把证件扫描件、恢复码截图、授权书、旧私钥备份等任意附件加密入库；可自定义显示名与备注，导出时仍用原文件名；查看 / 导出需二次验证。
- **备忘录**：用 Markdown 写私密图文（环境说明、操作步骤、账号线索）；插入图片时对可优化位图做无损压缩，正文与图片一并加密；可导出 Markdown 或 PDF。
- 两者都支持分组检索，并走同一套零知识云同步，换机用恢复密钥即可带走。

**文件保险库**：列表只显示元数据，导出或查看才解密。

![文件保险库：分组、用量与附件导出](docs/screenshots/09-file-vault.png)

**备忘录**：左侧列表（标签、置顶），右侧 Markdown 源码 / 对比 / 预览。

![备忘录：列表、标签与 Markdown 编辑](docs/screenshots/09-notes.png)
...
**关于与更新**：当前版本、更新源（GitHub 仓库或自定义清单）与网络代理。

![设置 → 关于与更新：版本、更新源与代理](docs/screenshots/10-settings-update.png)
...
文件保险库 ────── 加密附件、自定义名、二次验证导出
备忘录 ────────── Markdown 图文、无损压图、跟随同步
